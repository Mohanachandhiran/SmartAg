...
export default function FarmerLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    ...
    { name: 'Profit Simulator', path: '/farmer/profit-simulator', icon: <Calculator className="w-4 h-4" /> },
    { name: 'Mandi Prices', path: '/farmer/market-intelligence', icon: <TrendingUp className="w-4 h-4" /> },
    { name: 'Weather Forecast', path: '/farmer/weather', icon: <CloudRain className="w-4 h-4" /> },
    ...
    { name: 'Crop Scanner', path: '/farmer/crop-scanner', icon: <Activity className="w-4 h-4" /> },
    { name: 'Payments Feed', path: '/farmer/payments', icon: <CreditCard className="w-4 h-4" /> },
  ];
