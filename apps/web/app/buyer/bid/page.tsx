        ...
        {bids.map((b) => (
          <div key={b.id} className="bg-card border border-border p-5 rounded-xl shadow-sm flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-border pb-3 mb-4">
              <div>
                <span className="text-[9px] font-bold text-amber-600 bg-amber-400/10 px-2 py-0.5 rounded uppercase">
                  {t('buyer.bids.rankPrefix')} {b.rank} {t('buyer.bids.of')} 4 {t('buyer.bids.bidders')}
                </span>
              ...
              <div className="text-right">
                <span className="text-[10px] text-muted-foreground block font-bold uppercase">{t('buyer.bids.totalAmount')}</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
              <div>
                <span className="text-[10px] text-muted-foreground block uppercase font-bold">{t('buyer.bids.myBidRate')}</span>
                <span className="font-bold text-foreground">₹{b.myBid}/kg</span>
              </div>
              <div>
                <span className="text-[10px] text-muted-foreground block uppercase font-bold">{t('buyer.bids.highestBid')}</span>
                <span className="font-bold text-green-800">₹{b.maxBid}/kg</span>
              </div>
              <div>
                <span className="text-[10px] text-muted-foreground block uppercase font-bold">{t('buyer.bids.status')}</span>
                <span className="font-semibold text-foreground flex items-center gap-1">
                  <Hourglass className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                  <span>{t('buyer.bids.openBidding')}</span>
                </span>
              </div>
